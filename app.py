...
# Define agent
system_prompt = """You are a helpful personal assistant that specialises in managing my appointments and calendar. 
You have access to appointment management tools, a calculator, and can check the current time to help me organise my schedule effectively. 
...
if prompt := st.chat_input("Ask your agent..."):
    ...
    with st.spinner("Thinking..."):
        ...
        st.badge(f"Total Tokens: {response.metrics.accumulated_usage}", icon=":material/attach_money:", color="violet")
        st.badge(f"Execution time: {sum(response.metrics.cycle_durations):.2f} seconds", icon=":material/timer:", color="green")
        st.badge(f"Tools used: {list(response.metrics.tool_metrics.keys())}", icon="🔥", color="orange")
        st.json(response.metrics.get_summary(), expanded=False)
